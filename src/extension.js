...
function activate(context) {
    ...
    let addDisposable = vscode.commands.registerCommand('taskMaster.addTask', function () {
        vscode.window.showInputBox({ placeHolder: 'Add a new task' }).then(value => {
            
                vscode.window.showQuickPick(TASK_CATEGORIES, { placeHolder: 'Select a category' }).then(taskCategory => {
                    if (!taskCategory) {
    ...
    let updateDisposable = vscode.commands.registerCommand('taskMaster.updateTask', function (task) {
        if (task) {
    ...
    let deleteDisposable = vscode.commands.registerCommand('taskMaster.deleteTask', function (task) {
        if (task) {
